...
The Mac Mini is the collection/build runner because the CSV collection flow depends on the local browser/network environment. After the Mac Mini pushes updates, GitHub is the source of truth. Cloudflare Pages serves the generated static artifacts, and the Cloudflare Worker exposes the project at `radar.james.bregenzer.dev`, including the protected `/admin/` review console.
...
Normal review decisions should be made in the protected Cloudflare Worker admin console at `https://radar.james.bregenzer.dev/admin/`.
...
The public dashboard is served at `https://radar.james.bregenzer.dev/` through Cloudflare Pages/Worker routing. The public contribution history page is served at `/contributions/` on the same host.
