...
Review edits are different from collection runs. They should update the dashboard quickly, but they do not need to fetch new Trac data. For that reason, review commits to `data/reviews/reviews.json` trigger a GitHub Action that regenerates only the public dashboard and admin data export.

## Route ownership

```text
/radar/                    Public, static dashboard from docs/radar/index.html
/radar/contributions/      Public, static contribution history from docs/radar/contributions/index.html
/radar/admin/              Protected, Worker-rendered admin UI
```

The public dashboard and contribution history page are read-only. The dashboard contains scored ticket recommendations, public Trac links, and display-safe review metadata. The contribution history page turns public-safe review decisions into a visual record of tickets reviewed, tickets tested, component focus, and recent activity.
...
9. The `Refresh dashboard after review update` GitHub Action regenerates `docs/radar/index.html` and `docs/radar/admin-data.json` after review-only commits.
...
12. The Cloudflare Worker routes `/radar/` to the Pages origin and handles `/radar/admin/` directly.
...
```text
/radar/admin/ save review
  → Worker validates status/reason/notes
...
james.bregenzer.dev/radar/contributions/ WP Core Radar public contribution history
