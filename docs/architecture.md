...
Admin write-back should remain narrowly scoped to `data/reviews/reviews.json`. The Worker should not become a general repository editor.
...
- `review-server.py` renders the local development admin console only.
...
- `scripts/review-server.py` may remain in the public repo as local tooling, but it must not contain secrets and must not be exposed publicly.
